        ...
        <div class="container pt-3 pb-3">
            <b-card class="shadow-lg">
                <template slot="header">
                    <b-row class="pt-1">
                        <b-col md="6">
                            <h5 class="text-secondary">Peliculas</h5>
                        </b-col>
                    </b-row>
                </template>

                <b-card-body border-variant="white" header-bg-variant="white" header-text-variant="ssm"
                        ...
                        <b-col cols="12" class="mb-3" style="position: relative;">
                            <b-row>
                                ...
                                <b-col cols="6">
                                    <div>
                                        <b-input-group>
                                            <b-form-input id="search" pill
                                                :placeholder="'Buscar por ' + `${placehoderFilter}` + '...'"
                                                style="border: thin 0.4px gray;" v-model="search"
                                                @input="filter(search)">
                                            </b-form-input>
                                            ...
                                            <template #append>
                                                <b-dropdown variant="primary" text style="">
                                                    <b-dropdown-item
                                                        @click="() => getFilter('name')">Nombre</b-dropdown-item>
                                                    <b-dropdown-item
                                                        @click="() => getFilter('director')">Director</b-dropdown-item>
                                                    <b-dropdown-item
                                                        @click="() => getFilter('gender')">Género</b-dropdown-item>
                                                </b-dropdown>
                                            </template>
                                        </b-input-group>
                                    </div>
                            ...
                            <b-row>
                                <b-col>
                                    <FilterDates v-if="showSearch" :searchType="searchType"
                                        @hideComponent="hideComponent" @hideLoading="hideLoading"
                                        @showLoading="showLoading" @getMovies="getMovies"
                            ...
                            <b-row style="position: relative;">
                                ...
                                <template v-if="!isloading">
                                    ...
                                    <b-col cols="12" md="6" lg="4" class="mt-2 scroll-content"
                                        v-for="(movie, index) in movies" :key="index">
                                        <div v-if="movies.length != 0">
                                            <b-card pill draggable @dragstart="drag($event, movie, index)"
                                                bg-variant="white" text-variant="black" class="hover card-style mb-2"
                                                @click="Detail(movie.id)">
                                                <template #header>
                                                    <b-row>
                                                        <b-col cols="12"
                                                            class="d-flex text-truncate mt-1 justify-content-between align-items-center">
                                                            <b class="text-secondary">{{ movie.name }}</b>
                                                            <b-button variant="white" @click="changeStatus(movie.id)">
                                                                <b-icon class="delete-button" icon="x"
                                                                    style="color: #757577;" scale="1.5"></b-icon>
                                                            </b-button>
                                                        </b-col>
                                                    </b-row>
                                                </template>
                                                <b-card-text>
                                                    <b-row>
                                                        <b-col cols="12"
                                                            class="text-black d-flex mb-1  justify-content-between align-items-center">
                                                            <b class="text-secondary">Director:</b> <label>{{
            movie.director }}</label>
                                                        ...
                                                        <b-col
                                                            class="text-black mb-1 d-flex justify-content-between align-items-center"
                                                            cols="12">
                                                            <b class="text-secondary">Availability: </b>
                                                            <b-badge :class="movie.available ? 'success' : 'danger'">{{
            movie.available ? 'Disponible' :
                                    ...
                                    <div v-show="movies.length === 0">
                                        <b-row>
                                            <b-col cols="12" class="d-flex justify-content-center">
                                                <b-row>
                                                    <b-img :src="require('@/assets/withoutRecords.jpg')"
                            ...
                            <b-row v-if="!isloading">
                                <b-col cols="12" class="mt-1">
                                    <b-row>
                                        ...
                                        <b-col cols="6" md="4"
                                            class="d-flex justify-content-end order-md-5 align-self-center mb-4">
                                            <div class="d-inline justify-content-center align-items-right">
                                                <b-form-group class="text-right" label-cols="6" label-for="pages-select"
                                                    label-size="md" style="font-weight: bold">

                                                    <b-input-group size="lg">
                                                        <b-form-select id="pages-select" style="border: solid 1px gray;"
                                                            v-model="perPage" @change="getMovies()">
                                        ...
                                        <b-col cols="12" md="4"
                                            class="d-flex justify-content-center order-md-3 mt-4 align-self-center">
                                            <b-pagination v-model="currentPage" pills @input="getMovies()"
                                                :total-rows="rows" :per-page="perPage" size="md"></b-pagination>
                                        </b-col>
                                    </b-row>
                                </b-col>
                            </b-row>
            ...
            <SaveMovieVue @getMovies="getMovies()" />
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        async Detail(id) {
            // Encripta la variable 'id' convertida a cadena utilizando la función 'encrypt'
        ...
        async getMoviesByName(search) {
            if (search) {
                try {
                    this.isloading = true
                    // Realiza una solicitud para obtener películas por nombre encriptando el término de búsqueda.
                    const { status, data: { data } } = await movieServices.getMoviesByName(await encrypt(search))
                    if (status === 200) {
                        this.isloading = false
                        // Desencripta la propiedad 'data' y luego parsea el JSON resultante para obtener la propiedad 'content'.
        ...
        async getMovies() {
            ...
            try {
                ...
                if (status === 200) {
                    this.isloading = false
                    // Desencripta la propiedad 'data', luego parsea el JSON resultante
                    // para obtener las propiedades 'content' y 'totalElements'.
